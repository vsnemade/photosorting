...
public class FaceDetectionModel {
    ...
    public ImageElement getDetectedFaces(ImageElement imageElement) {
        try {
            ...
            if (Objects.nonNull(detectedFaces) && detectedFaces.getNumberOfObjects() > 0) {
    ...
    class FaceTranslator implements Translator<Image, DetectedObjects> {
        ...
        DetectedObjects processImageOutput(NDList list, List<String> className, float threshold) {
            ...
            // 1️⃣ Handle batched output: (1, N, 6)
            if (result.getShape().dimension() == 3 && result.getShape().get(0) == 1) {
                result = result.squeeze(0);   // now: (N, 6)
            }
            // 2️⃣ Handle output shaped like (N, 1, 6)
            if (result.getShape().dimension() == 3 && result.getShape().get(1) == 1) {
                result = result.squeeze(1);   // now: (N, 6)
            }
            // 3️⃣ Handle no detections (0,6)
            if (result.getShape().get(0) == 0) {
                return new DetectedObjects(Collections.emptyList(),
